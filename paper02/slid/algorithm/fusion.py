...
+0.056,基本为零——独立性前提**成立**。而在多通道攻击下 Fisher 的功效
(0.413) 远高于 Simes(0.187)与 minp(0.176),因为它累积**全部**通道的
证据,而 min 型统计量只看最极端的那一个。故默认改为 Fisher。
...
3) **合成不是免费的,必须同时报告逐通道结果。** 在只触碰时序的 A4 抢跑
   下,仅时序通道的功效是 0.110,而任何三通道合成都只有 0.042~0.060——
   合成把功效稀释了约 2.5 倍。合成的价值在**覆盖面**而非功效:换成
   多通道攻击后,Fisher 0.413、Simes 0.187,而仅时序通道仍停在 0.108。
   因此在线检测器应同时跑"合成"与"逐通道"两路,alpha 预算在两者间划分。
