...
def main():
    ...
    set_cjk(ax, ylabel=r"Dwell $\tau$")
    ax.set_title(
        r"Schematic: $\rho{=}0.22$ race stays inside the clock guard",
    ...
    ax.set_yticklabels([u"Accept", u"Reject"], fontproperties=FP_SM)
    set_cjk(ax, ylabel=u"Clock guard")
    ax.set_ylim(-0.55, 1.35)
    ax.text(n - 0.4, 0.58, u"All accepted", ha="right",
    ...
    set_cjk(ax, xlabel=u"Message index", ylabel=u"CUSUM $S_t$")
    fig.tight_layout()
