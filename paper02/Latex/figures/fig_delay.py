...
def main():
    ...
    set_cjk(ax, xlabel=r"Race amount $\rho$", ylabel=u"Detection rate")
    ax.set_title(u"(a) Weak-signal accumulation", fontproperties=FP_SM, loc="left")
    ...
    ax2.errorbar(rho, med, yerr=yerr, fmt="o-", color=C_OURS,
                 ...
                 label=u"Median / $p_{90}$")
    ax2.set_xlim(0.0, 0.55)
    ax2.set_ylim(0, 30)
    ax2.yaxis.grid(True, linestyle=":", color="#DDDDDD")
    ax2.set_axisbelow(True)
    set_cjk(ax2, xlabel=r"Race amount $\rho$", ylabel=u"Detection delay (messages)")
    ax2.set_title(u"(b) Counted in messages", fontproperties=FP_SM, loc="left")
