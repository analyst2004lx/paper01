...
恢复可行率由 \FeasRone\ 变为 \FeasRtwo;集内无指向集外的依赖边、集外占用的走廊、
时段与车辆逐条不变,两项各 \PassClosed\ 与 \PassInvar。把允许改写的集合换成
「决策时刻之后尚未结束的全部占用」、其余不变时,耗时与完工时间几乎不动,
改写比例却由 \ResFracCheapRtwo\ 升到 \ResFracAllFuture(逐格 \RAStabWTL,
Wilcoxon 符号秩双侧 \(p=\RAStabP\))。相对不改写已结束占用的热启动全局重调度,
有界修复耗时低两到三个数量级,但完工时间在全部预算档上更差、放宽预算不出现反超,
在完工时间上不能替代它。

主效应来自划界对象:上段的可行性恢复只因换了它,改路程序未动。
次效应来自影响集本身,即更少改写;但它已覆盖「尚未结束的全部占用」的约
\ClosureAliveFrac,两者接近时该收益趋于消失,故不作最小性声称。
集内改路、集外保持、失败后扩大范围都已见于既有工作;
本文主张的不是新的修复策略,而是划界对象。
\end{abstract}
...
把直接或间接必须作废、重写的记录纳入重调度范围，得到时空预约闭包（\STRC）。
...
\emph{而该划界在净收益上的价值属另一层}：本文对此的证据仅有 RA 一组对照
（第~\ref{sec:cheap}~节），即与「改写全部尚未结束的占用」相比少改写多少。
...
同一车间、同一预约表上，开工前的一体化排程用预约表构建无冲突方案~\cite{vanos2026exact,li2026framework,sun2023integrated}；本文在执行时刻用预约表定位损坏范围。二者时机不同，并不互相替代。
...
第~\ref{sec:strc}~章定义时空预约闭包，即预约影响集，说明其依赖边与对外封闭等性质
（图~\ref{fig:graphs}、\ref{fig:closure}）。
第~\ref{sec:repair}~章说明如何在影响集内为原车改路、集外保持原占用，
以及仍不可行时如何扩大允许改写的范围，并与全局重调度对照（图~\ref{fig:framework}）。
第~\ref{sec:experiments}~章报告可行性、算法耗时、占用改写比例与完工时间，
并用案例甘特把统计结果落到工序时间轴上。
...
处理执行期扰动的则没有预约表。
以下两处依据的是原文中的建模假设(原句照录于表~\ref{tab:refcheck}):
...
上面的缺口判断与第~\ref{sec:rw_aor}~节的定位都建立在对他人工作的转述之上,
而转述可能失真。为使读者不取原文即可核对,这些转述所本的原句列于表~\ref{tab:refcheck}。
列表的直接起因是其中一条没有通过核对:van Os 与 Basten~\cite{vanos2026exact}
的结论原先被本文转述为「以分解方法给出可证最优的基准」,而原文限定为多数算例取得最优、
少数因限时(time boxing)未能保证最优性。这一偏差的方向值得记下——
它替被引工作加强了结论,而被加强的恰好是本文要对比的基线。

\begin{table}[t]
\centering
\caption{定位所依赖的转述与被引原文对照。列出本文\emph{据以划出缺口}的五处转述,
每处给出被引工作的原句,供读者不取原文即可判断转述是否忠实。
第 4 行为一处已订正的转述:原表述强于原文,现已按原文收窄(见正文)。
原文均为英文,此处照录不译,以免二次转述再引入偏差。}
\label{tab:refcheck}
\footnotesize
...
\item \textbf{已完成占用不回溯。} 只改写 \(t_{\mathrm{end}}>t_{\mathrm{now}}\) 的尚未结束占用
...
其二,\textbf{只有 E4 一格必须两批合读}:主批次的三张同规模布局同属一族、几何差异很小,
在它们身上「指标太粗」与「算例集几何差异不足」这两种解释混在一起;而外部布局若没有
逐参数同口径的自建对照,也无从判断其影响集占比的变动幅度是否足够大。其余各节的结论由主批次
单独成立,第二批只增强其外部效度。
...
表~\ref{tab:e1} 把三个条件折进「C1 通过」一列合并计数,而三者的性质并不相同,故须拆开读;
逐格读数见数据文件 \texttt{e1\_miss.csv} 的
\texttt{n\_T\_impact} 与 \texttt{feasible\_after\_block} 两列。
其中 \(T_{\mathrm{impact}}=0\) 是定义~\ref{def:task_impact} 对 B 类约定的推论,
它把「工序依赖图按定义覆盖不到走廊阻断」显示出来,但本身不是发现。
非平凡的是后一列恒为假——排程\emph{确实}已经不可行。
两者合起来才构成反例。
...
读数直接回答了按影响集划界的净收益。预约影响集平均只比平凡上界少纳入 \(1-\ClosureShareMean\)
...
\(\ResFracAllFuture-\ResFracCheapRtwo\) 的全部落差,余下部分来自允许改写的占用集合变小后改路结果本身的
差异,本文不再细分:
...
这一层回答的是划界的净收益;「划界对象是否成立」则由第~\ref{sec:e1}~至~\ref{sec:e3}~节支撑。
放宽耗时约束后更短的完工时间仍来自热启动搜索(第~\ref{sec:e5}~节),
但那已不是毫秒档。
...
规模序列取工件数 \(2k\)、机器数 \(k\)、车数 \(k\),\(k=4,\dots,10\),即 \(8\times4\times4\) 到
...
\textbf{口径限制。} 这一规模序列只有一个拥堵档、一个算例随机种子(每档的算例本身由随机种子
...
本章读数按其支撑的贡献层归并;各条的局限、它威胁与不威胁哪一层,集中见第~\ref{sec:limitations}~节。

\textbf{对象层}(C1、C2:按什么划界、划出的集合是否唯一确定)由四组读数支撑。
走廊阻断下工序依赖图上的范围为空而原排程确已不可行(E1,\(\NPairs\) 对上 \(\PassMiss\));
预约影响集在依赖边上封闭、集外字段逐条不变(E2,\(\PassClosed\)、\(\PassInvar\));
同一套改路程序只换划界对象,恢复可行率即由 \(\FeasRone\) 变到 \(\FeasRtwo\)(E3),
可行与否因此来自划界对象,而不是改路程序的优劣;四类扰动与两种划界的交叉上 A/B 二分无例外,
且 A 类上预约影响集一致包含工序依赖图划界(E6)。
命题~\ref{prop:miss}--\ref{prop:outside} 给出对应的结构陈述,
其中命题~\ref{prop:outside} 仍只作充分条件。

\textbf{净收益层}(C3:这样划界换来什么)只由一组读数支撑:换用不按影响集限制范围的对照
(RA,与 R2 同一套改路、同样不改已经结束的占用),算法耗时与 \(C_{\max}\) 几乎不变,
改动比例却由 \ResFracCheapRtwo\ 升到 \ResFracAllFuture(逐格 \RAStabWTL,\(p=\RAStabP\);
第~\ref{sec:cheap}~节)。故毫秒级响应来自单遍改路而非影响集,
\textbf{少改写只能从这一对照读出}。与热启动全局重调度相比,有界修复在 \(C_{\max}\) 上
无一格更优,放宽预算也不出现反超(E5);E7 则排除了「对照耗时更长」这一疑问。

余下两组读数不支撑任何贡献,只给限定:规模扫描已扫至 \(\ScaleJobsHi\) 工件
\(\ScaleMachHi\) 机,以可改写集合为分母的影响集占比未随规模恶化(E8);
结构可预测性是负结果(E4)。第二批算例(\(\NPubPairs\) 对)只检验上述现象是否依赖
本文自选的布局,E1、E2、E3、E5 逐项复现。
...
收益幅度取决于影响集小于该平凡上界的程度,故两者接近时趋于消失。
...
本条\textbf{威胁}「按预约影响集划界的净收益」这一层,\textbf{不威胁}「须改写哪些占用由结构
唯一确定」这一层:少改写只能从与 RA 的对照读出,不能从与 R0+ 的对照读出(第~\ref{sec:cheap}~节)。

\item \textbf{对预约影响集,命题~\ref{prop:outside} 的保证只是充分条件,依赖边集未经独立的
穷尽检验,允许改写的集合不紧,其规模也不能由仅依赖路网的静态指标预测。}
影响集覆盖 \(t_{\mathrm{now}}\) 之后尚未结束占用的约 \(\ClosureAliveFrac\),本文不作最小性
声称(注~\ref{rem:minimality});净收益的幅度取决于影响集小于该平凡上界的程度,故两者接近时
收益趋于消失。静态割这一族指标已由第~\ref{sec:e4}~节排除,但该否定只针对「静态割能解释影响集
规模」,并不等于「路网几何与影响集规模无关」。
本条\textbf{威胁}对象层保证的强度与紧度,\textbf{不威胁}「按工序依赖图划界会给出空范围」
这一诊断读数。

\item \textbf{实现覆盖小于正文所述的问题范围:B 类扰动仅以改路响应,A 类另开换车与改派
但不改工序顺序,且由假设 A3 只做一次响应。}
放开工序顺序即向全局重调度退化,「有界」随之失去边界;扰动流、预测误差与滚动响应均未涉及。
规模一维已扫至 \(\ScaleJobsHi\) 工件 \(\ScaleMachHi\) 机且影响集占比未随规模上升,
但所扫档数不足以支撑任何渐近论断。
...
在其上一概无从定义(第~\ref{sec:protocol}~节)。
...
前缀解码对已结束占用的改写已在主批次消除(E5 的 \(18\) 个预算点越界 \(\RzeroPastLo\),
E7 上 RD 为 \RedecodeBad)。局限 1 余下的即 \(C_{\max}\) 本身,本文不以更换加权指标的方式规避。
若要缩小差距,须在不改写已结束占用的前提下作有限度的改派或改序,而此举会向全局重调度退化。

\textbf{二、使预约影响集的保证更完备、更紧,并为其规模建立刻画}(局限 \(2\))。
\emph{完备性}:接壤已并入让行边,重跑后试探泄漏为 \ProbeLeaks。
...
事后规则(允许改写占比,图~\ref{fig:worth}),但它随排程而变;须改用一族可在扰动前计算的指标:
...
后者提示了方向:起作用的似乎是\emph{需求的空间分布},而不是路网所提供的通行能力。
...
至于第二套独立实现,无须单独立项:上述任何一项复现时即可一并完成。
