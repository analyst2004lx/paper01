...
本文主张：当柔性作业车间用时间窗预约走廊来保证车辆无冲突通行时，扰动后的重调度范围必须定义在由走廊占用及其让行关系构成的时空预约图上，
而不是定义在以工序为节点的任务图上——否则走廊阻断、通行变慢等不改工序指派的扰动会因任务图上没有受影响工序而被漏报，占用失效在车辆之间的传播也划不进重调度范围。
为此我们提出时空预约影响闭包（STRC, Spatiotemporal Reservation Closure），即预约影响集：
把占用时段与扰动时窗重叠的走廊预约作为起点，再沿四类依赖向外收录——同走廊上的让行、同一辆车的后续占用、同一工件下一道工序的占用、同一机器上下一道工序的占用；
凡由起点沿这些关系直接或间接能够到达的预约，全部计入。
影响集以内的占用允许改写，以外的沿用原计划。该集合是上述关系下的传递闭包：再走一步也不会走出集合。
同走廊上谁先通过、谁让行，这一关系本身并不新鲜。本文不在排程时选择车辆的先后，而是在一份已经排好的计划上，按让行与接续关系，
把会因起点占用失效而必须改写的后续走廊占用全部收录——包括被挡住的他车占用、同一辆车的后续路段、以及同一工件或同一机器上下一道工序对应的运输占用——得到由这些依赖关系唯一确定的影响集；
而不是等重规划结束后，再根据实际改写了哪些走廊占用的时段或路径，事后倒推影响范围。
随后在影响集内为原车重新规划路径，集外保持原占用不变；若改路后仍对不上后续工序或仍有冲突，再按同一工件的后续运输、同一车辆的后续占用、直至全部未完成占用的顺序，
逐步扩大允许改写的范围。上述改路与扩大范围，只是把已经划定的影响集落到一份可行排程上；本文要主张的是影响范围应画在走廊占用及其依赖关系上，而不是提出一种新的修复策略。

实验取 $5$ 个算例、每个算例 $10$ 个随机种子，共 $50$ 组对照。各方法共用同一套走廊路网、时间窗路由与无冲突校验，扰动时刻、阻断窗与计时口径也相同。
对照包括：按工序依赖图划定重调度范围后再改路；以原排程为起点、在限定时间内做种群搜索的全局重解；
以及两档不做种群搜索的全局重算——一档将未完成占用整体后推到阻断结束之后，一档保持原指派与加工顺序、为全部未完成运输重新规划路径。
走廊阻断下，$50$ 组中工序依赖图给出的影响范围均为空，预约影响集均非空，且阻断后原排程均不可行。
若改路程序相同、只改影响范围的计算方式，则按工序依赖图划界时 $0/50$ 组恢复可行，按预约影响集划界时 $50/50$ 组恢复可行。
影响集在依赖边上向外封闭，即集内占用没有指向集外的依赖边；修复后集外每条走廊占用的走廊、时段与车辆均与原计划一致。这两项均在 $50/50$ 组上成立。
为单独检验预约影响集本身的作用，另设一对照：有界修复只改写影响集以内的走廊占用，影响集外沿用原计划；该对照则不再用影响集限制范围，把决策时刻尚未结束的走廊占用全部纳入改写。两者使用同一套改路程序，也都不得改写已经结束的走廊占用，差别仅在改写范围是影响集还是全部尚未结束的走廊占用。
二者耗时与完工时间几乎不变（有界修复对它的完工时间为赢/平/输 \WinVsAllFuture），但有界修复少改写约 $7$ 个百分点的走廊占用（逐组 \RAStabWTL，$p=\RAStabP$）。
由此可以分开两项读数：毫秒级响应来自对影响集内各段运输各做一次路径规划、不重开种群搜索；预约影响集本身带来的是稳定性，即更少改写原计划中的走廊占用。
有界修复耗时的中位数为 \MsRepairMed,ms。同一套实现中，保持原指派与顺序、为全部未完成运输重新规划路径的那一次全局重算，耗时的中位数仍高于一次有界修复；
唯一更快的是全局右移：不判断哪些占用受影响，把未完成占用整体后推到阻断结束之后，但其完工时间更差，改写的占用也更多。
相对不改写已完成占用的热启动全局重解，有界修复的耗时仍低两至三个数量级；在全部 $18$ 个时间预算点上，有界修复的完工时间均较差，不过差距已明显小于从零时刻起全部重排。
两边被改写的占用比例同在 \ResFracRtwo\ 量级——这一接近不能记成影响集带来的稳定性，必须与「只差允许改写哪些占用」的对照一起读。
将算例从 $8$ 工件 $4$ 台加工机器推至 $20$ 工件 $10$ 台加工机器，影响集占全部占用的比例不升反略降。
在全部 $18$ 个时间预算点上，有界修复的完工时间均劣于热启动的全局重调度，不存在随预算放宽而反超的交叉点。
完工时间上，有界修复因此不能替代全局重调度。走廊阻断发生后，若只按工序依赖图判断，重调度不会启动，原排程保持不可行；
若只标出损伤而不改写占用，冲突同样还在。全局重调度可以在观测到阻断后随时启动，但耗时在秒级以上。
当必须在毫秒内给出可执行排程时，需要的是与时间窗预约相匹配的局部恢复，而不是再开一次搜索。
影响集也不是恢复可行所必需的最小占用集合：它覆盖决策时刻尚未结束的走廊占用约 \ClosureAliveFrac；
稳定性收益的幅度与它未纳入、因而保持不动的占用多少成正比，故在影响集接近「全部尚未结束的占用」的算例上，这一收益近乎消失。
本文的贡献是把扰动后的重调度范围从工序依赖改画到走廊占用及其让行、接续关系上，并给出与该划界相匹配的局部恢复步骤。
\end{abstract}

\keywords{柔性作业车间调度, AGV 无冲突路由, 时空预约表, 影响闭包, 走廊阻断}
...
两篇带 AGV 运输、并以机器故障或插单为扰动的动态调度研究即按此划界——恢复范围分别取「故障后尚未加工的工序」与「按扰动传播分类后的工序与配送任务」。
...
上引两篇带 AGV 的动态调度研究没有出现「范围画在工序图上、因而漏掉走廊阻断」这一问题，
...
第二，划界对象是时空预约影响闭包（\STRC, Spatiotemporal Reservation Closure），即预约影响集：
...
在本文测到的每一个搜索时限上，全局重调度得到的 \(C_{\max}\) 都更小，有界修复没有在完工时间上反过来更优。
...
收录直接或间接必须作废、重写的记录，得到时空预约影响闭包（\STRC）。
...
按工序依赖图划界时走廊阻断 $0/50$ 恢复可行，按预约影响集划界时 $50/50$ 恢复可行，
可行与否来自划界对象，而不是改路程序的优劣；
有界修复以毫秒级耗时恢复可行，但完工时间 \(C_{\max}\) 在全部搜索时限上均劣于全局重调度，不存在交叉点；
相对「改写全部尚未结束的占用」的对照，耗时与 \(C_{\max}\) 几乎不变，
少改写的占用才可归因于影响集（第~\ref{sec:cheap}~节）。
四类扰动与两种划界的交叉中，工序图范围仅在走廊阻断与通行变慢上为空，
预约影响集均非空，且在机器或车辆故障上不小于工序图范围。
\end{enumerate}
...
第~\ref{sec:strc}~节定义时空预约影响闭包，即预约影响集，说明其依赖边与对外封闭等性质
...
区段不可用就是工序事件,不存在「范围画在工序图上、因而漏掉走廊阻断」。
...
（第~\ref{sec:containment}~节）；对外封闭是这一定义的推论，规模即该集合的大小。
...
把走廊预约与执行期扰动同时纳入的工作目前还没有。
模型中没有预约表、即使引入了预约表也把执行期故障排除在外:
Zhang 等~\cite{zhang2023energy} 并写明忽略 AGV 之间的碰撞；Sun 等~\cite{sun2023integrated}
用时间窗 Dijkstra 消解车辆冲突,却假定车辆始终可用、不考虑故障与充电,
并把设备维护与故障列为后续研究。

因此,缺口不是某篇已有文献把边界画错了,而是两个条件至今没有被融合:
...
又用时间窗记录走廊占用。此时会出现一类已有文献尚未处理的情形：
走廊阻断等扰动并不使工序无法执行，原指派仍然有效，
但与受扰时段重叠的走廊占用已经失效，原排程不可行。
按受影响工序划界会得到空范围，重调度不会启动；
持有预约表的一体化排程工作则通常不考虑执行期扰动。
本文研究这一问题，是因为现成划界覆盖不到这类损坏，需要先识别哪些占用必须改写。
...
工序依赖图 \(G_T=(T_T,E_T)\)（任务图）以工序为顶点,边为工件先后与同机先后;
...
(b)~预约依赖图 \(G_R\),顶点是走廊占用,从与阻断窗重叠的占用出发,沿四类依赖边纳入后续占用。}
...
执行期约定与物理设定如下。对照实验共用这些约定,以便把读数归因到重调度范围如何划定,而不是模型差异:
...
\begin{proof}
由定义~\ref{def:task_impact},走廊阻断不使任何机器、车辆或工序无法按原计划执行,故
\(T_{\mathrm{direct}}=\varnothing\)。\(\theta\)-步纳入的起点为空,故
...
\caption{预约影响集的构造示意。
(a)~与走廊阻断窗重叠的占用作为起点;
(b)~沿让行、接续边取传递闭包得到 \(\mathrm{Cl}(\mathrm{Seeds})\),集外占用保持原计划。}
...
则在恢复后的预约表 \(R'\) 中,每个集外任务标签对应的占用,其走廊、时段与车辆与 \(\mathcal{S}^0\) 一致。
\end{prop}
...
预约影响集的规模不是可调参数,而是由排程与路网决定的量:它随算例规模稳定上升
(第~\ref{sec:e1}~节)。因而「局部」是可以测量的,而不是人为设定的半径。
...
装卸点最小割与漏斗占比\emph{取值恒定},而影响集占比在路网不变、仅改机器落位时即大幅移动
...
预约影响集是 \(G_R^\circ\) 上一次广度优先,时间 \(O(|R^{\circ}|+|E_R^{\circ}|)\)。
...
此时不立刻改开全局搜索,而按下列顺序扩大 \(U\) 并重试同一套改路程序:
...
毫秒量级(第~\ref{sec:experiments}~节)。耗时随扰动比例 \(\varphi\)
并不单调(表~\ref{tab:scale}):比例小时初始影响集偏紧、集外保持的占用偏多,更常需要扩大范围;
比例大时初始 \(U\) 已较松、往往一轮成功,但单轮要改路的运输更多。
两种效应方向相反,故本文只报告耗时不随 \(\varphi\) 崩溃,不声称它随 \(\varphi\) 有可预测的走向。
...
有界修复在完工时间上并不优于 R0+。第~\ref{sec:e5}~节表明:R0+ 在最小预算下已占优,两条曲线不相交。
...
把「不按影响集限制范围」的做法直接实现出来即可回答——全局右移不判断哪些占用受影响,只把未完成占用
整体后推到阻断结束之后;而「重算但不按影响集划界」这一端,耗时更短的形态是按原染色体重解码一遍。
两者都在毫秒量级,构成有界修复在\emph{同一耗时档位内}的对照,
读数在第~\ref{sec:cheap}~节。

算法~\ref{alg:strc_repair} 与图~\ref{fig:framework} 对应:预约影响集给出 \(U\),
第 1 级改路,失败则按工件、车辆、全部尚未结束占用的顺序扩大 \(U\)。
...
\(777\)、\(31415\) 与 \(8\),共 \(\NPairs\) 个算例--随机种子对。
...
这五条对照按「是否划界 \(\times\) 重算多少」铺开:R1 与 R2 只差划界对象,
RS 不划界而整体后推,RD 与 R0+ 则重算全局而不按影响集划界,只差搜索多少代。
...
预约影响集的规模随算例规模上升(\(16.9\to 48.4\to 80\)\,余),但\(\mathrm{Cl}/|R|\) 在五个算例上
...
\begin{table}[t]
\centering
\caption{E2 包含性(\(\NSeeds\) 个随机种子/算例)。E2a 为结构闭合(泄漏 \(=0\)),
...
算法耗时中位数 \(\MsRepairMed\)\,ms、最大 \(\MsRepairMax\)\,ms。
...
左:工序依赖图上的影响域为空,而起点占用与预约影响集非空;
右:同一套改路、只换划界对象时 R1 可行率为 \(0\)、R2 为 \(100\%\)。}
...
\begin{table}[t]
\centering
\caption{E6 扰动类型 \(\times\) 划界对象(\(\NPairs\) 对/类,共 \(200\) 次划界测量)。
...
四点读数。第一,\textbf{A/B 二分在 \(200\) 次划界测量上无一例外}:两类 B 扰动按工序依赖图划界
...
结合第一点,预约影响集在这四类扰动上是工序依赖图划界的一致加强,而非另一种取舍。
...
第四,\textbf{修复一侧四类均满}。
阻断 \FeasESixBlock;通行变慢同口径 \FeasSlow。
...
相对「把尚未结束的占用全部纳入改写」这一平凡上界只少纳入一成。有界修复真正省下的是不改写已经结束的占用与不重开搜索,
...
即:\textbf{路网固定、只动机器落位,影响集占比的变动幅度可以超过换三张不同路网的幅度}
...
同一工具集另附的那族布局(即因允许对角移动而未纳入本批的一族)把装卸站放在网格边的中点,
...
谈不上,是\emph{无从谈起};第三个指标(每节点走廊数)虽有两个取值,与影响集占比的秩相关为
\(0\)。

据此可把读数收紧为:\textbf{影响集规模不由静态割决定}。这一族指标本是围绕哑铃布局那个可调的装卸点咽喉设计出来的,迁到真实布局上就
退化成常量,不具备解释力。
上面那组读数指出了替代方向:既然路网不变而落位一变,影响集占比就大幅移动,
那么决定影响集规模的不是路网\emph{能}提供多少条通路,而是\emph{需求落在哪里}——
...
在 \(C_{\max}\) 上反超。\textbf{该交叉点不存在}:有界修复的完工时间在全部预算点上较差。

表~\ref{tab:e5} 与图~\ref{fig:e5} 在两个算例上对随机种子 \(\{42,7,2024\}\) 扫描 R0+ 预算
...
「改动比例」为被改写的占用占全表之比。\(C_{\max}\) 一列 R0+ 全区间更优。}
\label{tab:e5}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
有界修复的完工时间全区间较差;算法耗时低两至三个数量级。与 R0+ 的改动比例则已同在 \ResFracRtwo\ 量级
...
走与 R0+ 同一套固定前缀解码。它是本文实现里\emph{耗时更短的一次全局重算}——种群搜索
每评估一个候选都至少要付这笔钱。它不改染色体,因而比 R0+ 耗时更短,也更少改派改序的自由度。
...
它同样不改写已经结束的占用,且已是最大允许改写集合,无需失败后扩大范围。
\end{itemize}

设置 RA 是为了回答:预约影响集既然已经覆盖尚未结束占用的约
\ClosureAliveFrac(注~\ref{rem:minimality}),那么\emph{按它划界还剩多少用}?
...
的尚未结束占用(逐格 \ClosureShareLo--\(1.00\),取到 \(1.00\) 的 \(4\) 个格上影响集\emph{就是}
...
\(+0.007\),一并记下。与此同时算法耗时几乎相同(\MsCheapRtwo\ 对 \MsAllFuture\,ms),
...
\ref{prop:outside} 承诺的那个量:集外占用的走廊、时段与车辆不变。}影响集的作用不在快、也不在缩短 \(C_{\max}\),
而在\emph{标出哪些占用不必改}——这解释了为什么幅度与影响集
排除掉多少成正比:在影响集接近全部尚未结束占用的算例上(\texttt{congested\_8x4x4},允许改写占比
\(0.94\))降幅只有 \(0.010\),而在影响集排除得多的算例上(\texttt{S8x4x4\_mid},\(0.91\))
降幅达 \(0.118\)。

把这一正比写成一条事后规则。图~\ref{fig:worth} 以允许改写占比
...
占比再升到 \(\WorthVanish\) 的 \WorthVanishN\ 格降幅为 \(0\)(影响集已是平凡上界,两法恒等)。
...
四法并读的结论是:在「不改写已经结束的占用、且响应在毫秒量级」这两个约束下,
\textbf{唯一比有界修复更快的做法(全局右移)在完工时间与改写比例上都更差;
不改写已结束占用的重解码在完工时间上仍略优,但更慢、改动也更多;
而在同等速度与同等完工时间下,不按影响集划界要多改约 \(6\) 个百分点的占用(相对多约一成二)}。
放宽算法耗时约束,更短的完工时间仍能从热启动搜索得到(第~\ref{sec:e5}~节),
但那已不是毫秒档。
...
(\texttt{congested\_8x4x4}, \(\NSeeds\) 个随机种子; R0+ 预算 \(2\)\,s; 有界修复含失败后扩大范围)。
两法可行率均为 \(100\%\); 有界修复耗时约 \(8\)--\(11\)\,ms,R0+ 约 \(2\)\,s;
...
步骤,本身已有成例(第~\ref{sec:rw_aor}~节)。本表的用途是说明,
即使在扰动比例升至 \(\varphi=1\)(全部未来工序受扰)时,按影响集划界的路径仍停留在十余毫秒量级,
算法耗时不随扰动规模崩溃。
...
「影响集」为预约影响集占全表之比;耗时取中位数。末列为 R2 对 RS 的 \(C_{\max}\) 胜负。}
\label{tab:instscale}
\small
...
\textbf{影响集占比不随规模增长。} 从 \ScaleCloHi\ 降到 \ScaleCloLo,趋势甚至是缓降的。
这是本节最要紧的一条:有界性并未随规模被稀释。但须说明这不是一个渐近主张——
七档不足以拟合任何增长率,能说的只是在这段区间内它没有退化。

\textbf{算法耗时随占用总数近似线性。} \(|R|\) 增至 \(2.7\) 倍时,R2 的耗时中位数从
...
而对不按影响集划界的右移,R2 的 \(C_{\max}\) 胜负从 \(7/1/2\) 变为最后三档的 \(10/0/0\)——
规模越大,「只改须改的占用」相对「整体后推」的优势越明显。
...
余下 \(1\) 点两法持平(\texttt{LyuL6\_8m},随机种子 \(2024\),预算 \(0.2\)\,s),
没有一个点上有界修复更优,交叉点仍不存在,而有界修复的算法耗时仍在毫秒量级、
R0+ 仍在秒量级。

这一批的意义不在于给出新的性能数字,而在于回答"这些现象是否只在本文自己设计的
布局上成立":B 类扰动使按工序依赖图划界得到空范围、预约影响集包含之、集外字段
保持原计划,这三件事在一批不由本文选定的拓扑上同样成立。唯一被这批算例改写的结论是
...
\item 与全局重调度相比,有界修复的 \(C_{\max}\) 全区间较差、无交叉;算法耗时为毫秒对秒,
...
(\ResFracShift\ 对 \ResFracCheapRtwo)上都较差;而同一实现里耗时更短的一次全局重算(原染色体重解码)比一次
有界修复还长 \RedecodeRatio\ 倍;RD 已走固定前缀,已结束占用越界 \RedecodeBad。
\item 同一批还把预约影响集单独隔离出来测了一次:换用「不按影响集限制范围、把尚未结束占用全部纳入改写」的对照(RA,
与 R2 同一套改路、都不改已经结束的占用),算法耗时与 \(C_{\max}\) 几乎不变(逐格 R2 赢/平/输 \WinVsAllFuture),
改动比例却由 \ResFracCheapRtwo\ 升到 \ResFracAllFuture——逐格 \RAStabWTL,
\(p=\RAStabP\)。故毫秒级响应来自单遍改路而非影响集;\textbf{少改写只能从这一对照读出},
且降幅与影响集排除掉多少成正比;样本内事后规则是允许改写占比超过 \WorthThresh\ 时
平均降幅从 \WorthLowMean\ 落到 \WorthHighMean(图~\ref{fig:worth})。
\item 算例规模扫描(E8)把梯子推到 \(\ScaleJobsHi\) 工件 \(\ScaleMachHi\) 机:影响集占比
从 \ScaleCloHi\ 缓降到 \ScaleCloLo(有界性未随规模稀释),算法耗时随占用总数近似线性
...
本文指认了一类工序依赖图覆盖不到的损坏,并把重调度范围改到走廊占用及其让行、接续关系上,
得到由这些关系唯一确定的预约影响集(\STRC)。
在 \(\NInst\) 算例 \(\times\) \(\NSeeds\) 个随机种子共 \(\NPairs\) 对上:
走廊阻断时按工序依赖图划界 \(\PassMiss\) 为空,而原排程确已不可行;
同一套改路程序下只替换划界对象时,按工序依赖图划界 \(\FeasRone\) 恢复可行,按预约影响集划界 \(\FeasRtwo\);
影响集的结构闭合性 \(\PassClosed\) 成立,集外占用的走廊、时段与车辆逐条不变 \(\PassInvar\) 成立;
四类扰动 \(\times\) 两种划界的覆盖矩阵上 A/B 二分无例外,且 A 类上预约影响集一致包含
工序依赖图划出的范围。

让行关系、集内改路而集外保持原计划、以及失败后扩大允许改写的范围,
分别已见于铁路 alternative graph、无冲突多车路径与 Local Rescheduling
(第~\ref{sec:rw_claim}~节)。这些步骤用来落实影响集。本文主张的是:
工序依赖图按定义不含走廊占用之间的让行关系,因而覆盖不到只使占用失效的扰动;
改到预约依赖图上之后,须改写的占用由既定先后唯一确定,而不是一个启发式半径。

把允许改写的集合从预约影响集换成「\(t_{\mathrm{now}}\) 之后尚未结束的全部占用」,
而保持同一套改路程序、都不改已经结束的占用、协议也不变时,算法耗时与完工时间几乎不动,改动比例却由
\ResFracCheapRtwo\ 升到 \ResFracAllFuture(逐格 \RAStabWTL,\(p=\RAStabP\);
第~\ref{sec:cheap}~节)。故毫秒级响应来自单遍改路、不重开搜索;
\textbf{少改写只能从这一对照读出,落实的是命题~\ref{prop:outside} 承诺的量:集外占用保持原计划}。
收益幅度与影响集排除掉多少成正比,因而在影响集接近平凡上界时趋于消失。
...
\item \textbf{完工时间上全区间劣于不改写已结束占用的热启动全局重调度,且不存在交叉点。}
在 \(18\) 个预算点上无一例外。对照已走固定前缀解码,已结束占用不可改写的审计为
\(\RzeroPastLo\)(第~\ref{sec:e5}~节);E7 上 RD 越界同样为 \RedecodeBad。
差距比从 \(t=0\) 重排时小得多,但方向没变:有界修复换来的是算法耗时,交出的是完工时间。
第~\ref{sec:cheap}~节已排除「对照耗时更长」:同一实现里耗时更短的一次全局重算
比有界修复更慢,且已走固定前缀;少改写只能从与 RA 的对照读出,不能从对 R0+ 读出。

\item \textbf{对预约影响集,保证仍只是充分条件,边集未经独立穷尽检验,
允许改写的集合不紧,规模也不可由只看路网的指标预测。} 四者分述。
...
逐条比对集外字段(E2b)\emph{不足以}兜住这类错配:同一注记的后半段记录了
一次只发生在影响集\emph{内部}、因而 E2b 查不到的、对已结束占用的改写,它要靠一项独立的
审计才暴露出来。因此本文对该类缺陷的检出依赖于「事后想到该查什么」,
而非一个完备的检查。

\emph{(ii) 依赖边集并不穷尽物理耦合,但已观测到的那条接壤通道已经关掉}。
E2b 满通过检验的是修复实现与允许改写集合是否一致,不是四类边覆盖了一切传播通道。
...
少纳入的不足一成。这一条的\emph{幅度}已实测(第~\ref{sec:cheap}~节的 RA):
那不足一成的差别把改动比例降了 \RAStabMean(逐格 \RAStabWTL,\(p=\RAStabP\)),
而算法耗时与 \(C_{\max}\) 不变。限制不在于「划界无用」,而在于\textbf{收益幅度不大且与
影响集排除掉多少成正比}——在影响集接近全部尚未结束占用的算例上降幅仅 \(0.010\)。
...
装卸点最小割与漏斗占比都没有做出可复现的区分;换用外部来源布局后这一点变得更确定——这两个指标
在整批外部算例上\emph{取值恒定}(两张路网都把装卸站放在网格角点,四邻接下角点恰有两条
出边),而影响集占比在\emph{同一张}路网上仅换机器落位就变动 \PubCloSpread(第~\ref{sec:e4}~节)。
这条否定只针对「静态割能解释影响集规模」,\emph{不是}「路网几何与影响集规模无关」——后者需要更多张
...
但那条梯子只有一个拥堵档与一个算例随机种子,\(\ScaleKs\) 档也不足以支撑任何渐近论断,
更大规模与更多结构下的行为仍未知。

\item \textbf{外部参照缺位:考虑执行期扰动的这一类研究没有可换的公开基准,算例出处只外部化了一半,
读数亦未在第二套实现上复现。}
带无冲突运输的柔性作业车间已有若干公开算例族,但\emph{执行期扰动}这一类没有:
...
工序依赖图给出空范围与 R1 全败在 \(150\) 格上复现;R2 第 1 级可行率分别为
\WeightLogRtwo、\WeightLuRtwo、\WeightSplitRtwo。
空范围对边权不敏感;第 1 级可行率只在二分边上掉了一格(\WeightSplitRtwo),
