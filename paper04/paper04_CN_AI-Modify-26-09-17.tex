...
[ref\_check: key=zhang2023energy,tang2026dynamic; claim="两者的恢复范围分别取故障后尚未加工的工序、与按扰动传播分类后的工序与配送任务"; need=quote]。
...
已纳入的占用 \(\PassClosed\)
（第~\ref{sec:e3}~节）、边界消融的差分试探泄漏 \ProbeLeakPairs。
...
并以分解方法给出可证最优基准
[ref\_check: key=vanos2026exact; claim="该工作自称给出 provably optimal 的基准"; need=quote];
两阶段方案先固定排产再消解车辆冲突~\cite{sun2023integrated};
亦有学习层排产、路径层消冲突、并以有限反馈连接的框架~\cite{li2026framework}。
但这些工作回答的是开工前如何排出可行且质量好的无冲突排程。

按检索所及,把走廊预约与执行期扰动同时纳入的工作尚未见到
(检索范围:<待填:数据库>;时间窗:<待填:起止年份>;检索式:<待填:检索式>)。
模型中没有预约表、即使引入了预约表也把执行期故障排除在外:
Zhang 等~\cite{zhang2023energy} 并写明忽略 AGV 之间的碰撞
[ref\_check: key=zhang2023energy; claim="该工作在模型中明确忽略 AGV 之间的碰撞"; need=quote]；
Sun 等~\cite{sun2023integrated}
用时间窗 Dijkstra 消解车辆冲突,却假定车辆始终可用、不考虑故障与充电,
并把设备维护与故障列为后续研究
[ref\_check: key=sun2023integrated; claim="该工作假定车辆始终可用、不考虑故障与充电,并把设备维护与故障列为后续研究"; need=quote]。
...
\ScaleCloLo--\ScaleCloHi\ 间波动而不趋向 \(1\)。因而「局部」是可以测量的,
而不是人为设定的半径。
...
有界修复在完工时间上并不优于 R0+。第~\ref{sec:e5}~节表明:R0+ 在最小预算下已占优,
在那 \(6\) 个算例--随机种子格上未观察到交叉。
...
(\(8\) 核 \(16\) 线程)、\(64\)\,GB 内存、Windows 10,解释器为 CPython <待填:版本号>。
...
局部重调度做法,由本文重新实现,<待填:哪几个部件逐条对应原文、哪些由本文重写>,
而改路与重放部件与 R2 共用同一份实现,以保证两者只差划界对象;
...
\(0.073\)、在 RA 上为 \(0.062\)。因 \(50\) 格中有 \(18\) 格两法恰好持平,配对差
...
而 \(\mathrm{Cl}/|R|\) 在五个算例上都落在 \(0.56\)--\(0.59\),\emph{未能}区分三个 \(\texttt{S8x4x4}\) 布局。
...
而不是把未来切得很小。第~\ref{sec:e1}~节按全表统计的 \(\mathrm{Cl}/|R|\) 落在 \(0.56\)--\(0.59\)
...
\(\mathrm{Cl}/|R|\) 亦落在 \(0.56\)--\(0.59\) 的窄带内,同样未能区分布局。

\textbf{换一批不由本文设计的布局,这条负结果的成因才分得清.}
三张同规模布局同属哑铃一族,只差并行通道数,即只差容量、不差几何;"指标太粗"与
...
左侧同时换了机器台数与落位,右侧三张自建布局均为 \(4\) 机、只换路网;故这个不等号支持的是
「机器配置这一维不可忽略」,而非「落位单独的效应大于路网」。两张路网上都成立的是
...
在全部 \(18\) 个点上无一例外。但这 \(18\) 个点\emph{不是} \(18\) 份独立证据:它们来自
\(2\times 3=6\) 个算例--随机种子格,每格三个预算点共用同一个 R2 定点(R2 不随预算变),
算例这一维更只有 \(2\) 个样本;故本节不作显著性断言,只报「这 \(6\) 个格子上未观察到交叉」。

\begin{table}[t]
\centering
\caption{E5 与不改写已结束占用的热启动全局重调度(\(3\) 个随机种子均值)。
「改动比例」为被改写的占用占全表之比。本表共 \(2\) 算例 \(\times\) \(3\) 随机种子
\(\times\) \(3\) 预算点;\(C_{\max}\) 一列 R0+ 在每个预算点上更优,但独立格数只有 \(6\)。}
\label{tab:e5}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
百分之百对六成;但在这 \(6\) 个格子上仍无交叉。缺口里原先混着的「改写已经结束的占用」已经被拿掉,
...
Wilcoxon 符号秩双侧 \(p=\RAStabP\)。方向高度一致,\emph{幅度不大}——唯一的反例格上 R2 反而多改 \(0.007\),一并记下。与此同时算法耗时几乎相同(\MsCheapRtwo\ 对 \MsAllFuture\,ms),
...
\ref{prop:outside} 承诺的那个量:集外占用的走廊、时段与车辆不变。}影响集的作用在于\emph{标出哪些占用不必改}——这也解释了幅度为何随影响集排除得多而变大:
在影响集接近全部尚未结束占用的算例上(\texttt{congested\_8x4x4},允许改写占比 \(0.94\))
降幅只有 \(0.010\),而在排除得多的算例上(\texttt{S8x4x4\_mid},\(0.91\))降幅达 \(0.118\)。
...
有界修复与 R0+ 的算法耗时(对数轴)与完工时间随 \(\varphi\) 变化。}
...
\textbf{影响集占比不随规模增长。} 七档在 \ScaleCloLo--\ScaleCloHi 之间波动、并不单调
(首末两档略降,中段两档回到 \(0.53\) 以上)。这是本节最要紧的一条:占比始终在半数上下,
有界性并未随规模被稀释。但须说明这不是一个渐近主张——
七档不足以拟合任何增长率,能说的只是在这段区间内它没有退化。

\textbf{算法耗时随占用总数增长,略快于线性。} \(|R|\) 增至 \(2.7\) 倍时,R2 的耗时中位数从
\(2.9\) 增至 \ScaleMsHi\,ms、即 \(3.6\) 倍,仍在十毫秒量级。第 1 级改路的可行格数为 \ScaleFeasFirst;
...
\PubEoneFracLo--\PubEoneFracHi,整体略低于主批次逐算例的 \(0.56\)--\(0.59\),
两者同处半数上下的量级;本文未对这一偏低作归因。
与全局重调度的权衡亦无变化:\PubEfiveNoCross\ 个预算点上 R0+ 的 \(C_{\max}\) 更优,
余下 \(1\) 点两法持平(\texttt{LyuL6\_8m},随机种子 \(2024\),预算 \(0.2\)\,s),交叉点仍不存在。
...
\item 与全局重调度相比,有界修复的 \(C_{\max}\) 在全部预算点上较差,这 \(6\) 个算例--随机
种子格上未观察到交叉;算法耗时为毫秒对秒,
...
在 \ScaleCloLo--\ScaleCloHi 间波动而不单调(有界性未随规模稀释),算法耗时略快于线性
升至 \ScaleMsHi\,ms,与重解码的耗时比单调拉大到 \ScaleRatioHi\(\times\)。
这是一段区间内的读数,不是渐近主张。
...
外部路网上换三种机器配置,影响集占比即变动 \PubCloSpread(为自建路网间极差 \SelfCloSpread\ 的
约四倍;两侧变动的因子数不同,故这只支持「机器配置这一维不可忽略」),而 LU 最小割与漏斗占比
