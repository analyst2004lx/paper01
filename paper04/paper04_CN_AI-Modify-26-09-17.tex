...
且放宽预算不出现反超（\(\EfiveCells\) 个算例--随机种子格，预算维上无交叉点）；
...
本文\emph{不}主张这一组合在文献中不存在——那是一条无法由本文核实的全称否定。
这里只报一个可逐篇核对的事实:在本节所引的这些工作里,两个条件没有同时出现,
有预约表的把执行期故障排除在外,处理执行期扰动的则没有预约表。
以下两处是直接引原文假设的:
...
在 \(C_{\max}\) 上反超。\textbf{该交叉点在预算这一维上不存在},而原因不是样本不够,
...
故本节不作显著性断言。预算维上的非存在由上面那条单调性给出,与样本量无关;
...
\(\EfiveCells\) 个算例--随机种子格上,预算维不存在交叉点(R2 为定点、R0+ 单调不增),
