...
%% E4 同参数对照。要点有两条且方向相反:闭包占比在外部布局上的极差是自建族的约四倍
%% (故先前测不出区分有算例集的责任),但 LU 最小割与漏斗占比在这批外部布局上取值**恒定**
%% (该族把装卸站放在网格角点,四邻接下角点恰两条出边),故这两个指标在那里无从谈起。
...
\newcommand{\PubEfiveNoCross}{18/18}
...
第~\ref{sec:protocol}~节第二批的 \NPubInst\ 张外部布局把两者分开了(表~\ref{tab:e4pub})。
同一 LU 割阻断协议下,外部布局的闭包占比中位落在 \PubCloLo--\PubCloHi、极差
\PubCloSpread;而\emph{逐参数同口径}的三张自建布局只落在 \SelfCloLo--\SelfCloHi、极差
\SelfCloSpread。即:\textbf{闭包占比确实随布局显著变化,幅度约为自建族的四倍}——先前
测不出区分,相当一部分责任在算例集,并不全在指标。

\textbf{但指标并未因此得救,反倒暴露出一个更硬的问题.}
在这 \NPubInst\ 张外部布局上,\textbf{LU 最小割恒为 \(2\)、漏斗占比恒为 \(0\)}。
这不是巧合,而是结构性的:这一族布局把装卸站放在网格角点,而四邻接网格的角点恰有两条
出边,这个割因此被钉死。同一工具集另附的那族布局(即因允许对角移动而未收入的一族)把
装卸站放在网格边的中点,其出边数是 \(3\)——换言之,\textbf{这个指标在真实布局上只取
到极少数几个由"装卸站摆在哪种网格位置"决定的值},并不随几何连续变化。
一个取值恒定的量不可能解释一个极差达 \PubCloSpread\ 的量,
所以在这批布局上,这两个指标连"相关性弱"都谈不上,是\emph{无从谈起};第三个指标
(每节点走廊数)虽有两个取值,与闭包占比的秩相关为 \(0\)。

据此可把原先那句含糊的观察收紧为一条明确的否证:\textbf{闭包规模不由装卸点处的静态割
决定}。最小割这一族指标本是围绕哑铃布局那个可调的 LU 咽喉设计出来的,迁到真实布局上就
退化成常量,不具备跨布局的解释力。因此本文\textbf{不}把「闭包规模由争用结构决定」列为
贡献;要预测闭包规模,需要刻画的应是 \(t_{\mathrm{now}}\) 邻域内走廊占用的\emph{时序}
密度,而不是静态拓扑的连通度——这一方向列入后续工作(第~\ref{sec:future}~节)。
...
两组逐参数同口径,只差布局来源。外部布局的闭包占比极差约为自建族的四倍,而 LU 最小割
与漏斗占比在外部布局上\emph{取值恒定},故这两个指标在那里无从解释差异。
边权为本文补齐的等权值,故本表不与外部文献的参照值比较。}
\label{tab:e4pub}
\small
\begin{tabular}{@{}llcccccc@{}}
\toprule
出处 & 布局 & 机器 & 节点/走廊 & LU 割 & 漏斗占比 & \(\mathrm{Cl}/|R|\) & 极差 \\
\midrule
外部 & \texttt{LyuL2} & \(4\) & \(16/23\) & \(2\) & \(0\) & \(0.512\) & \\
外部 & \texttt{LyuL3} & \(5\) & \(16/23\) & \(2\) & \(0\) & \(0.305\) & \\
外部 & \texttt{LyuL4} & \(6\) & \(25/38\) & \(2\) & \(0\) & \(0.385\) & \\
外部 & \texttt{LyuL5} & \(7\) & \(25/38\) & \(2\) & \(0\) & \(0.422\) & \\
外部 & \texttt{LyuL6} & \(8\) & \(25/38\) & \(2\) & \(0\) & \(0.460\)
  & \(\PubCloSpread\) \\
\midrule
...
的读数与主批次\emph{逐项一致},没有一条结论因换布局而改变:
...
\PubEoneFracLo--\PubEoneFracHi,与主批次的 \(0.54\)--\(0.57\) 同量级。
与全局重解的权衡亦无变化:\PubEfiveNoCross\ 个预算点上 R0+ 的 \(C_{\max}\) 全部更优,
交叉点仍不存在,而有界修复的响应时间仍在毫秒量级、R0+ 仍在秒量级。
...
\item 结构可预测性(E4)是负结果,且换用外部来源布局后结论更硬:闭包占比在布局之间
确实相差显著(极差 \PubCloSpread,约为自建族的四倍),但 LU 最小割与漏斗占比在那批布局上
\emph{取值恒定},无从解释这一差异。故闭包规模不作可预测性主张。
\item 把布局出处换成外部来源后(\(\NPubPairs\) 对),E1/E2/E3/E5 的读数逐项复现,
无一条结论因换布局而改变(第~\ref{sec:pubrepro}~节)。
\end{itemize}
...
\item \textbf{闭包规模的结构可预测性未能建立,且已知不由装卸点处的静态割给出。}
最小割与漏斗占比都没有在布局之间做出可复现的区分;换用外部来源布局后这一点变得更确定
——这两个指标在那 \NPubInst\ 张布局上\emph{取值恒定}(该族把装卸站放在网格角点,四邻接下
角点恰有两条出边),而闭包占比却相差显著(第~\ref{sec:e4}~节)。下一个候选刻画应是
\(t_{\mathrm{now}}\) 邻域内走廊占用的时序密度,本文未做。

\item \textbf{布局出处已外部化,但边权与工件数据仍自建;这一支没有可换的公开基准。}
...
\textbf{给闭包规模找一个时序刻画}:第~\ref{sec:e4}~节已把静态割这一族指标排除——它在
真实布局上取值恒定,而闭包占比相差显著。下一个候选是 \(t_{\mathrm{now}}\) 邻域内的走廊
占用时序密度,例如受扰走廊在其繁忙窗口内的重叠预约条数、以及该走廊在让行图上的出度。
这类量随排程而变,不再是纯拓扑量,因而也需要重新界定「可预测性」问的是什么:是给定布局
预测闭包规模,还是给定布局\emph{与当前排程}预测它。
\textbf{把时间标定也外部化}:布局出处已解决一半(第~\ref{sec:limitations}~条 \(9\)),
但逐段行驶时间仍是本文补的等权值。要补齐另一半,需要一份同时发布\emph{排他路网与逐段
时长}的算例族;若无现成来源,退一步的做法是在等权之外再扫若干条非均匀边权,检验本文的
结论是否对边权口径敏感。
