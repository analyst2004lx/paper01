"""案例甘特中文版：example_3x3x2 / seed42 走廊阻断下，修复前后工序时间轴。"""
...
OUT = os.path.join(HERE, "fig_strc_case_CN")
...
def _draw_panel(ax, result, closed_ops, *, t_now, block, title, show_legend=False):
    ...
    colors = {True: "#1f4e79", False: "#b0b8c0"}
    ...
    ax.text((t0 + t1) / 2, -0.95, f"阻断于 {cid}",
...
def main() -> None:
    ...
    _draw_panel(
        ...
        title=(f"(a) 修复前  $C_{{\\max}}={bundle.makespan:.0f}$  "
