...
%% article.
\begin{abstract}
The widely used of the Industrial Internet of Things (IIoT) has
...
system demonstrates that MBDF achieves an average detection rate of
...
HAI ICS benchmark dataset---further confirms that MBDF achieves 82.6\% DR on
...
the generalizability of the framework beyond the AGV--robot
...
that conflicts with the milliseconds latency budgets of
industrial protocols (e.g., PROFINET, Modbus-TCP)~\cite{ref14,ref15}.
...
\subsection{Traffic-Based Detection Methods}
...
\midrule
%
% HMM-IDS: latent (hidden) states -> NOT directly observable (xmark);
%   no physical mask; fixed threshold; not state-dependent;
%   requires O(n^2 T) Viterbi decoding -> NOT O(n^2) per step (xmark);
%   trained on normal data only -> unsupervised (cmark).
HMM-IDS~\cite{ref_ourston}
  & \xmark & \xmark & \xmark & \xmark & \xmark & \cmark \\
%
% Bayes-IDS: uses discrete observable states (cmark);
%   no physical feasibility mask (xmark);
%   has adaptive thresholds based on global Bayesian posteriors (cmark),
%   but NOT per-state / state-dependent (xmark);
%   online O(n^2) inference (cmark);
%   requires labeled attack samples for threshold calibration -> NOT unsupervised (xmark).
Bayes-IDS~\cite{ref_spathoulas}
  & \cmark & \xmark & \cmark & \xmark & \cmark & \xmark \\
%
% Markov-Flow: operates on statistically discretized continuous traffic
%   metrics, NOT domain-knowledge-derived discrete operational states (xmark);
%   no physical mask (xmark); fixed threshold (xmark); not state-dependent (xmark);
%   online Markov inference O(n^2) (cmark); unsupervised (cmark).
Markov-Flow~\cite{ref_ye}
  & \xmark & \xmark & \xmark & \xmark & \cmark & \cmark \\
%
% BN-Inference: uses discrete state variables (cmark);
%   no physical feasibility mask (xmark);
%   adaptive probabilistic inference (cmark), but global, not state-dependent (xmark);
%   batch belief propagation -> NOT O(n^2) per-step online (xmark);
%   trained on normal data -> unsupervised (cmark).
BN-Inference~\cite{ref17,ref22}
  & \cmark & \xmark & \cmark & \xmark & \xmark & \cmark \\
%
% FMM: monitors aggregate traffic statistics, NOT discrete operational states (xmark);
%   no physical mask (xmark); fixed statistical threshold (xmark);
%   not state-dependent (xmark); lightweight online computation O(n^2) (cmark);
%   no labeled data required -> unsupervised (cmark).
FMM~\cite{ref24,ref25}
  & \xmark & \xmark & \xmark & \xmark & \cmark & \cmark \\
%
% SMLC: operates on discrete state/feature representations (cmark);
%   no physical mask (xmark); fixed trained classifier boundary (xmark);
%   not state-dependent (xmark); online classifier inference O(n^2) (cmark);
%   requires labeled attack samples -> NOT unsupervised (xmark).
SMLC~\cite{ref9,ref10,ref11}
  & \cmark & \xmark & \xmark & \xmark & \cmark & \xmark \\
\midrule
%
% MBDF: satisfies ALL six criteria by design (see Sections 3--4).
\textbf{MBDF (Ours)}
...
\texttt{AGV\_idle} to \texttt{AGVAtRobot} without an
intermediate transition state \texttt{MovingToRobot}), inflating false negatives
...
is either an AGV or a robot arm. The production floor is
...
\begin{equation}
  \mathcal{S}_{AGV} = \{\texttt{AGV_Idle},\,\texttt{AGVAtOrigin},\,
  \texttt{MovingToRobot},\,\texttt{AGVAtRobot},\,\texttt{MovingToOrigin}\}
  \label{eq:agv_states}
\end{equation}
For robotic arms:
\begin{equation}
  \mathcal{S}_{Robot} = \{\texttt{Idle},\,\texttt{Producing}\}
  \label{eq:arm_states}
\end{equation}
...
\texttt{AGV\_Idle} to \texttt{AGVAtRobot}). We encode these constraints
...
\midrule
TaskAssigned      & AGV_Idle        & MovingToRobot  & AGV \\
ArrivalAtRobot    & MovingToRobot   & AGVAtRobot     & AGV \\
MaterialDelivered & AGVAtRobot      & MovingToOrigin & AGV \\
ArrivalAtOrigin   & MovingToOrigin  & AtOrigin       & AGV \\
AtOriginReady     & AtOrigin  		& Idle       	 & AGV \\
MaterialDelivered & Idle            & Producing      & RobotArm \\
ProcessingDone    & Producing       & Idle           & RobotArm \\
\bottomrule
\end{tabular}
\end{table}
...
signaling anomaly, whereas rare states (e.g., \texttt{AGVAtRobot},
...
AGV--robot arm domain, we apply the framework to the
...
state to the structurally equivalent AGV/RobotArm state in
Scenario~A.}
...
Analogous to the AGV/RobotArm feasibility mask $F$
...
(AGV/RobotArm vs.\ steam turbine) provides strong evidence
...
performance metrics across Scenario~A (AGV/RobotArm
...
advantage of MBDF is not specific to the AGV/RobotArm
...
and attack scenarios (Scenario~A: AGV/RobotArm simulation).
...
simulation scenario (Scenario~A: 25-task AGV+RobotArm system,
...
transferable not only across different AGV/RobotArm
configurations but also across structurally distinct
physical domains (AGV/RobotArm vs.\ steam turbine),
...
\midrule
A\ (25 tasks, 3 AGV\,+\,1 RobotArm)  &
  0.73 & 0.893 & $[0.55,\,0.95]$ & ---     \\
B\ (40 tasks, 4 AGV\,+\,2 RobotArm)  &
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.3}
\caption{Ablation study results on Scenario~A (AGV/RobotArm
...
experimental validation on both a simulated AGV--robot
