...
%%     把它降格为定语"低开销的"之后,冒号自然消失,标题回到"命名一个构造物"的方法型形状。
%%  2. "低开销"而非"降本"。本文最小化的是完工时间,标题里出现"cost/成本/降本"会被
%%     第一时间读成目标函数,而这里指的是单次评价的计算开销。英文同理用 Low-Overhead。
%%  3. "路由内嵌评价"对应英文 Routing-in-the-Loop Evaluation,指适应度取自下层真实
%%     无冲突路由之后的完工时间(第 4.1 小节的评价通路),不是"对路由本身做评价"。
%%
%% 对应英文题名(投英文刊时使用):
%% A Low-Overhead Closed-Loop Bilevel Framework with Routing-in-the-Loop Evaluation
%% for Integrated Flexible Job Shop Scheduling and Conflict-Free AGV Routing
\title[低开销的路由内嵌评价闭环调度]{面向柔性作业车间调度与 AGV 无冲突路由一体化的低开销路由内嵌评价闭环双层框架}
...
之后的完工时间,使让行延误进入搜索所优化的目标,而不再是事后校验;决策通路上承重的机制
...
\NSeeds{} 个随机种子、同挂钟预算与配对 Wilcoxon 检验($p$ 值按族做 Holm 校正,少数例外在正文
逐处标明,并报 Hodges--Lehmann 伪中位数与 $95\%$ 置信区间),与一个四级基线阶梯比较,其中前
两级分别对应两篇已发表工作的结构。把拥堵搬进搜索目标使完工时间降低 \GainLoopRule{}
($p\PLoopRule$,伪中位数 \HLLoopRule{},CI \CILoopRule{});在闭环内部将规则派车替换为
预约表感知派遣再降低 \GainProbeLoop{}($p\PProbeLoop$,CI \CIProbeLoop{});端到端相对
文献结构降低 \GainEndToEnd{}(\WinEndToEnd{} 胜 \LoseEndToEnd{} 负,$p\PEndToEnd$)。

两条机制都显著,但量级与稳健性相差悬殊:闭环是主效应(\GainLoopRule{}/\GainLoopProbe{}),
且在 \NCases{} 个算例格中有 \CellsSigLoopAdj{} 格在校正后仍自身显著;预约表感知派遣是小得多的
次效应——开环下值 \GainProbeOpen{}($p\PProbeOpen$),闭环内再降 \GainProbeLoop{}
($p\PProbeLoop$)——它只在合计口径上显著,逐格校正后\emph{无一格}显著,因此本文不就
...
(争用强度覆盖 \ContentionLo{} 至 \ContentionAHi{})、车臂比与柔性。
...
\CellsSigLoopAdj{}/\NCases{} 个算例格上校正后仍自身显著;预约表感知派遣是量级小得多的次效应,
...
"知道它在哪里有效"分开:后者需要的种子数远多于前者,而这一点在同类工作中很少被区分。
...
比较每格)。显著性标记按 \textbf{Holm 校正后}的 $p$ 给出,族为一个效应的 \NCases{} 个格:
...
cell, and its sign alternates across the four fleet-ratio levels.}
...
\NCases{} 个格),空心为不显著。闭环增益 \CellsSigLoopAdj{} 格显著且大体随争用上升;试探增益
校正后\emph{无一格}显著,在车臂比族的四个水平上符号还交替——因此本图要读的是两条曲线
\emph{可信度}的差别,而不是试探那条曲线的形状。}
...
稳健的:$\NCases$ 个算例格中 $\CellsSigLoopAdj$ 格在 Holm 校正后仍自身显著。预约表感知派遣
在开环与闭环下的合计口径上都显著($\GainProbeOpen$,$p\PProbeOpen$;$\GainProbeLoop$,
$p\PProbeLoop$,$95\%$ 置信区间 \CIProbeLoop{}),但逐格校正后无一格显著,因此它是一个
"平均起来成立"的效应,本文不就它在何种结构上兑现作主张。端到端 $\GainEndToEnd$ 接近独立乘积 $\GainIndepProduct$(近加性,无正交互);
...
\textbf{(1) 算例族,以及为何只能自建。} 全部算例格共用一个规模,我们不就机制如何随规模变化
作任何主张;异质度 $H$ 也不在这一族的扫描之内——它固定在 $0.3$,其四个水平的扫描来自
第~\ref{sec:whichcongestion}~小节那批规模更小的独立算例,故"收益不随 $H$ 上升"是在\emph{那个}
规模上得到的。布局族本身还有一处须明说的不齐整:它由\emph{两段}构成,哑铃三档之间只动网络
容量、不动行驶距离,后两档换成网格与错落网格,拓扑与距离一并变了(最远行程由 \LegDumbbell{}
升到 \LegGrid{},见第~\ref{sec:instances}~小节)。因此只有哑铃三档之间的差值可以干净地归给
容量,五档不宜按争用强度连成一条曲线来读;要把"收益如何随容量变化"测到网格一侧,需要在网格上
另做一组容量旋钮(例如按边限流),这超出本文的算例设计。用以刻画该族的争用强度也是一个粗糙的
一维摘要:它给出"让行占行驶时间的比例",不区分其中有多少是指派可以绕开的——这与上述两段拼接
一起,构成布局族上收益与争用强度并不严格同序的两个可能原因。

之所以只能自建,是因为带无冲突运输的这一支尚无可用的公开基准。通行的 FJSP\_PT 基准把行驶时间
发布为一张常数矩阵,第~\ref{sec:instances}~小节已就实际取用的那一族查明,
\PubNLayoutAsym{}/\PubNLayout{} 张矩阵不对称因而不可能是任何无向走廊网络的 $t^{*}$,其中
\PubNLayoutBad{} 张连最短路闭包都不是:既然不存在与之一致的通道网络,争用在这类算例上就无从
定义,在其上运行本文方法必须先删掉本文所讨论的那条约束。第~\ref{sec:external}~小节仍做了这个
退化比较并如实报告——退化之后争用不存在、本文两条机制全部失效,故它检验的是上层搜索而非本框架。
少数带显式路网的公开算例则各有不可比之处:\cite{vanos2026exact} 预约的是\emph{节点}(两车不得
同时占据同一节点或交换节点),而本文的走廊是双向排他资源、预约的是\emph{边},在其算例上比较的
将是两次模型翻译而不是两个方法;\cite{lyu2019approach} 在冲突语义上与本文同构(双向路网、
边排他),但它\emph{另外}要求任一节点同时只容一辆车,本文模型是它的\emph{松弛},在其算例上可能
给出在其模型下并不可行、完工时间却更低的解,数字不可比,且其规模较大的一组未发表逐段行驶时间
(该文唯一给出逐段时长的算例,其布局图画法与那几张相同却取了三种不同的边权,可见图是示意而非
按比例,故边权是真未知)。后者的正面意义在于:"双向走廊 $+$ 边排他"这一建模选择在文献中已有
先例,并非为本文方便而设。

\textbf{(2) 逐格统计功效。} 每格 $\NSeeds$ 个种子足以分辨闭环效应(Holm 校正后
$\CellsSigLoopAdj/\NCases$ 格自身显著),却不足以分辨预约表感知派遣:该效应校正前有
$\CellsSigProbe/\NCases$ 格显著,校正后为 $\CellsSigProbeAdj/\NCases$ 格。后者的量级只有数个
百分点,与本问题的种子间离散度同量级,因此我们只能在 $\NPairs$ 个配对的合计口径上确认它为正,
而即便在那里它也只是刚刚站住($95\%$ 置信区间 \CIProbeLoop{},下端离零不到一个百分点),
更无法定位它在哪些结构上兑现。这不是一个可以靠改写论述绕开的限制,而是需要把种子数提高约一个
数量级才能解除的限制;第~\ref{sec:costlaw}~小节的代价律所预言的车队规模上界,因此在本文中
仍是未被检验的预言。我们把这一条单列,是因为在同类文献中,逐格符号被当作适用区间来叙述是
常见做法,而按本文的功效水平那样做会是在读噪声——本文自己的前一版也正是这样写过柔性族,
是逐格 $p$ 值的族内校正把它挡了下来。

\textbf{(3) 绝对水平:上层搜索、松下界与单一实现。} 所有配置共用一套代码,这消除了配置之间的
实现方差,但绝对完工时间反映的是该实现的质量。第~\ref{sec:external}~小节的退化对标给出了外部
标定:$\PubNMatch{}/\PubNInst{}$ 个算例上取得与文献逐位相同的完工时间,$10$ 个小型算例全部命中
可证最优且无一次运行低于可证最优值,故它不是一个弱实现;但间隙随规模恶化,四个最大算例平均
\PubGapBig{},且把算力提到 \PubBudgetRatio{} 倍只换回 $\PubBudgetGainPP$ 个百分点,故这是能力
边界而非预算问题。这一条\emph{不}威胁主效应,首要的理由是口径而非量级:上层搜索的绝对强弱是
各档\emph{共有}的,本身无法制造出各档之间的差值,而本文主张的正是差值(差值是否与搜索强度有关
则是另一个我们没有检验的问题,已列入第~\ref{sec:future_work}~小节)。量级只是旁证,且我们不
夸大它:闭环增益 \GainLoopRule{} 约为最大算例那档间隙 \PubGapBig{} 的两倍、为 \PubNInst{} 个
算例平均间隙 \PubGapMean{} 的七倍左右,并非高出一个量级。但它确实意味着,几个百分点量级的
次效应(如 \GainProbeLoop{})所处的尺度与实现自身离最优的距离\emph{相当}——这与局限 (2) 是同一
件事的两个侧面:次效应之所以脆弱,既因为噪声与它同量级,也因为它落在实现离最优的距离之内。

自建算例上则给不出这类标定,因为复合下界是松的,与最好解的间隙中位数为 \LBGapMed{};它的作用
是数量级核查与回归防护,不是最优性证书,而这么大的间隙无法区分"搜索失败"与"下界松弛"。退化
对标顺带给出了一次分解:在有可证最优值的那 \PubNOpt{} 个公开算例上,同一个下界距可证最优的
中位数为 \PubLBSlack{},距本文最好解的中位数为 \PubLBTotal{},逐算例作差后"搜索欠缺"这一份最大
只有 $\PubLBResidMax$ 个百分点——间隙几乎全部来自下界自身的松弛。这个分解不能直接搬回自建算例
(那里存在争用,而该下界完全忽略争用,只会\emph{更}松),故我们仍不就自建算例上解与最优解的
接近程度作定量主张;但原因已收窄为缺少那些算例的最优值,而不再是两种成因无从判断。同理,公开
算例没有争用,因而也未标定下层路由与预约表这一半代码的绝对水平,而它恰是本文新增的那一半。
...
\textbf{把绝对水平在两端测准。} 大规模一端,第~\ref{sec:external}~小节已测出上层搜索在
...
这指向"陷在不同的局部结构里"而非"尚未收敛"。小规模一端则是精确解:已有工作表明基于逻辑的
Benders 分解可为该问题类给出精确解~\cite{vanos2026exact},它能确定间隙中有多少是可回收的——这
正是本文因下界过松(局限 (3))而无法回答的问题——也能检验那些在启发式搜索下不划算的机制在搜索
更强时是否依然不划算;可行的做法不是把本文方法搬到对方算例上(冲突语义不同构,见局限 (1)),
而是把该精确方法的冲突模型改写为走廊排他的版本后,在本文的小规模算例上求解,改写的正确性需
单独验证,故这是一项独立工作而非一次补充实验。两端都与本文的机制结论\emph{正交}:上层变强会
同时抬高阶梯上的每一级,而各级之差才是本文的主张;但它会改变次效应能否从噪声中被分辨出来,
因此也是把局限 (2) 解除掉的一条路。

\textbf{把次效应的适用区间测出来。} 本文能确认预约表感知派遣平均为正,却说不出它在哪些结构上
