...
\toprule
& & \multicolumn{2}{c}{$C_{\max}$(时间单位)} & & 评价次数比 & 单次评价加速比 \\
\cmidrule(lr){3-4}
算例 & 争用强度 & 关 & \textbf{开} & $\Delta C_{\max}$ & (开/关) & (关/开) \\
\midrule
...
\midrule
\emph{配对合计}(60 配对) & --- & --- & --- & $-5.60\%^{***}$ & \multicolumn{2}{r}{50/0/10,\ $<10^{-4}$} \\
\midrule
\multicolumn{7}{@{}l@{}}{\footnotesize HL 伪中位数与 $95\%$ CI:$-4.72$ $[-6.34,-3.55]$\,\%} \\
\bottomrule
\end{tabular}
