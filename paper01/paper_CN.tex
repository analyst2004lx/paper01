...
算例共 $4$ 个工件、$2$ 辆 AGV：焦点工件 J1 的一道工序可选 M1（$t^P=3$）或 M2（$t^P=14$），另三个背景工件只能上 M3，成品须运回上下料站。
通往快臂 M1 的唯一通路是一条排他干道，慢臂 M2 挂在自有支路上；背景工件反复穿越干道，把它占满。

图~\ref{fig:motivating} 中须区分两类读数，二者不可相互替代。
第一类为焦点工件 J1 的无阻挡作业链：J1 自 LU 出发，加工完成后运回 LU，行驶时间取最短路，不计其余车辆。
指派 M1 时路径为 $\mathrm{LU}\to v_1\to v_2\to\mathrm{M1}$，单程 $1+\tau+1=8$，往返 $16$，计入加工 $3$ 得 $19$；
指派 M2 时路径为 $\mathrm{LU}\to v_1\to\mathrm{M2}$，单程 $1+2=3$，往返 $6$，计入加工 $14$ 得 $20$。
在理想矩阵下，快臂在该比较中占优当且仅当 $19<20$，故 $\tau=6$、$t^P_{\text{fast}}=3$ 时 $t^P_{\text{slow}}$ 的最小取值为 $14$。该异构度恰为次序反转得以发生的下界，而非为放大效应而设定。
图~\ref{fig:motivating}(a) 底部给出的即为此链，与 (b) 中柱高所表示的整例 $C_{\max}$ 不是同一量。

第二类为整例完工时间，由与全文同一解码器求得，对应于图~\ref{fig:motivating}(b) 的柱高。
$C_{\max}$ 定义为最晚一件成品返回 LU 的时刻，既不是四件工件工时之和，也不是将 $19$ 或 $20$ 与背景工件的加工时间叠加。
四件工件均自 LU 出发并运回；J2、J3、J4 仅可在 M3 上加工（去程与回程各为 $8$，加工各为 $3$）；两辆 AGV 允许并行。

在常数矩阵下，车辆互不干扰，走廊让行不进入评价。
若将 J1 指派至 M1，则 $C_{\max}=41$：第一波中 J1$\to$M1 与 J2$\to$M3 并行，各经去程 $8$、加工 $3$、回程 $8$，同于 $t=19$ 返回 LU；
第二波中 J3、J4 均运往 M3，同于 $t=19$ 出发、去程 $8$ 于 $t=27$ 抵达，J3 在 $27$--$30$ 加工后于 $38$ 返回，J4 待 M3 空闲至 $30$、加工至 $33$、再经回程 $8$，最晚返回时刻为 $41$。
若将 J1 指派至 M2，则 $C_{\max}=54$：J1 经去程 $3$、加工 $14$、回程 $3$，AGV1 在 M2 处等待至 $t=17$，于 $t=20$ 返回 LU；J2 仍于 $19$ 返回。
此后两车不再保持齐头并行，J4 抵达 M3 的时刻为 $43$，返回 LU 为 $54$。矩阵评价仍选择快臂，但 $41$ 与 $54$ 不能由 $19$ 对 $20$ 直接推出。

将同一对指派改由无冲突路由评价。
指派 M1 时 $C_{\max}=95$：J1 的无阻挡链仍为 $19$，但其去程占用干道 $[1,7)$、回程占用 $[12,18)$，J2 在干道入口让行 $17$，抵达 M3 的时刻为 $25$；随后 J2--J4 在干道上串行，J4 返回 LU 的时刻为 $95$。
指派 M2 时 $C_{\max}=60$：J1 取支路而不占用干道，J2 仅在 $\mathrm{LU}$--$v_1$ 让行 $1$；背景工件之间仍发生让行，J4 返回 LU 的时刻为 $60$。此时慢臂更优。
机器指派与工序顺序均未改变，改变的仅是评价方式。

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig/fig_motivating_CN.pdf}
\Description{两幅。左图是小车间网络：通往快臂的唯一通路经过排他干道，慢臂挂在自有支路上，底部写出焦点工件自身的无阻挡链 19 对 20。右图是同一对指派的整例完工时间：常数矩阵下快臂以 41 对 54 胜出，无冲突路由下变为 95 对 60、慢臂胜出。}
\caption{动机算例:同一个指派决策,两种评价给出相反次序。
(a) 通往快臂 M1 的唯一通路是排他干道($\tau=6$),慢臂 M2 接在不经过干道的短走廊上;三个只能上 M3 的工件反复穿越干道。
底部是焦点工件 J1 从 LU 出发再运回的无阻挡链：M1 为路径 $\mathrm{LU}\to v_1\to v_2\to\mathrm{M1}$ 往返 $2(1+6+1)=16$ 加加工 $3$ 得 $19$，M2 为 $\mathrm{LU}\to v_1\to\mathrm{M2}$ 往返 $2(1+2)=6$ 加加工 $14$ 得 $20$；用来标定能发生反转的最小异构，不是 (b) 的柱高。
(b) 柱高是整例最后一件回 LU 的时刻($4$ 工件、$2$ 辆 AGV,与论文同一套解码器)，含 J2--J4 上 M3 的运输与加工，不能写成各件工时之和。
常数矩阵不含让行,$C_{\max}$ 为 $41$ 对 $54$,选快臂;无冲突路由下同一对指派变为 $95$ 对 $60$,选慢臂。
差值由 $-13$ 翻为 $+35$。
...
橙色圆点是剪枝与胜者路径复用的开关两档（引出线标出开档）——命题~\ref{prop:speedup_equiv} 保证\emph{不改变输出}即可沿该线移动，故这段位移只是算力节省。}
\label{fig:protocol}
