...
承重的是\emph{预约表感知的车辆派遣}:对预约表做两段式试探来选车,取代按理想最短路估计
到达时刻的规则派车。为使它在同等算力下\emph{低开销},本文给出两项可证不改变
输出的优化:可采纳下界剪枝与胜者路径复用。

在 \NCases{} 个受控算例格上(共用单一规模,争用强度自 \ContentionLo{} 至 \ContentionHi{},
后者为全族最大值),以 \NSeeds{} 个随机种子、同挂钟预算与配对 Wilcoxon 检验($p$ 值按族做
Holm 校正,并报 Hodges--Lehmann 伪中位数与 $95\%$ 置信区间),与一个四级基线阶梯比较,
其前两级对应两篇已发表工作的结构。相对开环阶梯,把拥堵搬进
搜索目标使完工时间降低 \GainLoopRule{}($p\PLoopRule$,伪中位数 \HLLoopRule{},CI
\CILoopRule{});在闭环内部把规则派车换成预约表感知的车辆派遣再降低
\GainProbeLoop{}($p\PProbeLoop$,CI \CIProbeLoop{});端到端相对文献结构降低
\GainEndToEnd{}(\WinEndToEnd{} 胜 \LoseEndToEnd{} 负,$p\PEndToEnd$),接近两者独立时
应有的 \GainIndepProduct{}。

两条机制的量级与稳健性相差悬殊:闭环是主效应,\CellsSigLoopAdj{} 个算例格在校正后仍
自身显著;预约表感知的车辆派遣是小得多的次效应,只在合计口径上显著,逐格校正后
\emph{无一格}显著。因此本文不就"它在哪类结构上兑现"、也不就机制如何随规模变化作主张;
端到端呈近加性,未检出正交互。
\end{abstract}
...
\keywords{柔性作业车间调度, 无冲突路由, 一体化调度, 走廊争用, 闭环双层框架,
路由内嵌评价, 预约表感知的车辆派遣, 低开销, 时间窗预约}
...
时,去一台更慢但连通性更好的机械臂可能更优",在它们的模型里连表述都无法完成。第~\ref{sec:Chapter_II}~节
把这两半逐篇落到具体文献上。
...
以上,因此在固定算力下它必须与它所挤占的那些额外评价次数竞争。这笔交换的汇率正是本文要给出
的东西:\cite{li2026framework} 的作者本人把"将该机制扩展为完全迭代的回路"列为未来工作,但
没有给出把它做成回路要付多少、又能换回多少。
...
而不是一个假想的弱基线。为使它在同挂钟预算下\emph{低开销},本文给出两项可证不改变输出的优化:
基于可采纳下界的剪枝,以及胜者路径的复用。

我们同样实现并考察了两条更具雄心的接口——为走廊时段定价,以及回传结构化的冲突凭证以制导
局部搜索——但它们在同挂钟预算下的增益均未获证实。本文如实报告这一点,把两者作为受控负对照
(第~\ref{sec:Chapter_V}~节);它们不是本文的结论,而是本文设计取舍的正当理由。
